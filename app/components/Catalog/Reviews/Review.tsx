...
import { Button, ButtonGroup, Card, CardContent, CardOverflow, Chip, Stack, Typography } from "@mui/joy";
    ...
    return (
        ...
        >
            ...
            <Stack direction='column' sx = {{ p: 0.2 }}>
                <Typography level='title-sm' sx = {{ mb: 1 }}>@{props.username}</Typography>
                <Typography level='body-xs' sx = {{ color: (props.username == user?.username ? 'white' : 'inherit' )}}>{props.content}</Typography>
            </Stack>
            </CardContent>
            {(props.recommends.length > 0) && (<Stack
                direction='row'
                spacing={1}
            >
