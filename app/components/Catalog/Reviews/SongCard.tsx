    ...
    async (default_return = "") => {
      ...
      try {
        let retrievedArtwork = await getArtwork(
          props.songCardContent?.album.title,
          props.songCardContent?.album.artist.name
        );
    ...
    <Box>
      <Card variant="plain">
        <CardOverflow>
          <AspectRatio ratio={2}>
            <img
              src={image ?? "/img/cassette.png"}
              alt={props.songCardContent?.album.title}
            />
          </AspectRatio>
          <Stack
            direction="row"
        ...
        <CardContent sx={{ mt: 0 }}>
          ...
          <Box sx={{ justifyContent: "flex-end" }}>
            <Typography
              level="title-md"
              sx={{ pl: "35%", textAlign: "right", textWrap: "wrap" }}
            >
              {props.songCardContent?.album.title}
            </Typography>
            <Typography
        ...
        <CardOverflow
          variant="soft"
          sx={{ bgcolor: "background.level1", borderRadius: 0 }}
        >
