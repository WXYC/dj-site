...
export default function CatalogResultContextMenu({
  ...
  const runRotation = async (bin: Rotation | null) => {
    const menuAlbumId = catalogSlice.selectors.getResultContextMenu(
      store.getState() as RootState,
    )?.albumId;
    const ok = await setRotationBin(bin);
    const stillOpen =
      catalogSlice.selectors.getResultContextMenu(store.getState() as RootState)
        ?.albumId === menuAlbumId;
    if (!stillOpen) return;
    if (!ok) {
