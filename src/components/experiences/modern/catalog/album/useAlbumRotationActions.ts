...
/**
 * The one write path for rotation membership on a catalog surface, shared by
 * the album modal's classify control and the catalog row's context menu —
 * replacing two hand-rolled orchestrations that disagreed on copy and on
 * whether a re-bin retired the prior entry first.
 *
 ...
 * The add-before-retire ordering, and why it is the one that fails safe,
 * lives in the shared `addThenRetire` helper this hook drives; `kill`
 * remains for removing a single entry without adding a replacement.
 *
  ...
  const setRotation = async (
    ...
    try {
      const outcome = await addThenRetire(
        // Guarded above.
        bin ? () => addRotationEntry({ album_id: album.id!, rotation_bin: bin }).unwrap() : null,
        activeEntries.map((entry) => entry.rotation_id),
        killQuiet,
      );

      if (outcome.step === "add-failed") {
        // Nothing landed, so the generic copy is accurate. A non-JSON failure
        // already has the middleware's own toast; only an otherwise-unreported
        // one obliges this hook to speak — the retire failures below likewise.
      ...
      const unretired = outcome.retireFailures.length;
      if (unretired === 0) {
        if (bin) {
      ...
      if (outcome.retireFailures.some(({ error }) => isUnmessagedHttpError(error))) {
        ...
        const remaining = (outcome.added ? 1 : 0) + unretired;
        toast.error(
          outcome.added
