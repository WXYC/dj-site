...
export default function CatalogEntryArtistAutocomplete({
  ...
  const commitInput = useCallback(
    (raw: string, options: ArtistAutocompleteExisting[] = baseOptions) => {
      applyCommitResult(
        resolveArtistInputCommit(raw, options, allowCreateArtist),
  ...
  const handleBlur = useCallback(async () => {
    ...
    if (
      ...
      options = (result.data?.artists ?? []).map(toExistingOption);
    }
    commitInput(q, options);
  }, [
    baseOptions,
    commitInput,
    debouncedQ,
