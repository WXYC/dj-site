...
NEXT_PUBLIC_ROTATION_ADMIN_ENABLED=false
...
- `NEXT_PUBLIC_ROTATION_ADMIN_ENABLED` — gates the Rotation Admin surface (rotation card management, the free-form filing bench) reached from the catalog admin screens. Defaults to OFF; set to `"true"` or `"1"` to enable. Helper: `isRotationAdminEnabled()` in `lib/features/rotation/flags.ts`. Build-time, like every `NEXT_PUBLIC_*` flag — inlined at build, so flipping it needs a rebuild/redeploy. Flip on per-environment once Backend-Service is serving the rotation cards and filings endpoints there. See WXYC/dj-site#1480.
