...
- `NEXT_PUBLIC_CLASSIC_LIBRARIAN_NAV_ENABLED` — gates the librarian entries in the classic navigation bar (card-catalog admin, missing releases, rotation). Defaults to OFF; set to `"true"` or `"1"` to enable. Helper: `isClassicLibrarianNavEnabled()` in `lib/features/catalog/flags.ts`. The screens stay URL-reachable and server-gated whether or not this is set: it controls **discoverability**, not authority, so a mid-rollout deploy doesn't advertise a half-built menu to a working librarian. Flip on once every screen in the `docs/architecture.md` librarian URL map has landed. See WXYC/dj-site#1163.
