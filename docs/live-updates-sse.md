...
1. **Pre-launch**: BS-1, BS-2, BS-3, BS-4 land and deploy to staging. `wxyc-shared` PR lands. dj-site PR lands with both flags OFF in production.
...
3. **Dashboard rollout**: flip on staging, then production. All authenticated DJ tabs now subscribe. Observe `WXYC/Backend-Service/SSE/ClientCount` for a few days.
