...
- **Shipped after v0** (dj-site#1118): `insert` SSE events. BS broadcasts new `entry_type='track'` rows (BS#1888) as `LiveFsInsertEvent` — the raw `FlowsheetEntryResponse` allow-list projection, NOT the converted read shape. The listener converts via `convertV2Entry`, splices the row into `getInfiniteEntries` page 0, and schedules the debounced `Flowsheet`+`NowPlaying` invalidate as the consistency backstop — for genuinely NEW ids only. An already-cached id is the sender's own broadcast echo and is skipped entirely (no merge, no refetch): the sender's mutation pipeline owns that row, and a refetch per self-echo replays only the recorded pageParams, collapsing optimistic rows past the page size out of the list during rapid adds. For new rows the backstop matters because: it keeps NowPlaying in step with a brand-new row, carries non-broadcast marker rows (breakpoints/talksets) the way the pre-insert unknown-id repair path used to, and re-syncs offset-based pagination after page 0 grows. Bursts past `INSERT_PATCH_BURST_LIMIT` per debounce window skip the per-row patch and lean on that refetch. An insert against an uninitialized cache emits `sse_cache_uninitialized` and schedules the invalidate rather than silently no-oping.
...
- `posthog.capture('sse_unknown_event_id', { surface, event_type, payload_id, current_show_id })` as a counter for tuning the unknown-id debounce policy. `surface` distinguishes dashboard vs live consumer, `event_type` is `'update' | 'refetch' | 'insert'`, `payload_id` lets ops identify whether specific rows are systematically out-of-cache, and `current_show_id` (read from the loaded cache, or `null` if none) helps spot whether unknown ids cluster around show boundaries. The insert path additionally emits `sse_cache_uninitialized` when an insert arrives before `getInfiniteEntries` has ever loaded (public `/live`, auth-gated dashboard skip) — the lane where RTK's `updateQueryData` would otherwise no-op silently.
