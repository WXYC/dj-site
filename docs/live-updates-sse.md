...
- **Out** (deferred to v1+): cross-tab leader election (each tab opens its own EventSource for now), reconnect-driven explicit refetch (5-minute safety poll covers gaps instead), `delete` SSE events from Backend-Service, `whoIsLive` SSE (no Backend-Service emission for show/DJ state yet), Sentry adoption in dj-site (since resolved: errors now report to Sentry *and* PostHog through `lib/error-reporting.ts` — see docs/adr/0008; the `sse_*` analytics events below are unaffected).
- **Shipped after v0** (dj-site#1118): `insert` SSE events. BS broadcasts new `entry_type='track'` rows (BS#1888) as `LiveFsInsertEvent` — the raw `FlowsheetEntryResponse` allow-list projection, NOT the converted read shape; the listener's guard enforces the track-only contract, so any other `entry_type` surfaces as `sse_unknown_event_type` instead of a corrupt splice. The listener converts via `convertV2Entry`, splices the row into `getInfiniteEntries` page 0, and schedules the debounced `Flowsheet`+`NowPlaying` invalidate as the consistency backstop — for genuinely NEW ids only. Two lanes skip the splice and never schedule a `Flowsheet` refetch (a refetch replays only the recorded pageParams, so a per-self-add refetch collapses the sender's optimistic rows past the page size out of the list during rapid adds): (1) an already-cached id — either the sender's own broadcast echo (the mutation pipeline owns that row; merging the raw pre-enrichment echo would downgrade enriched fields) or a receiver whose refetch raced the frame — nudges `NowPlaying` only, which is load-bearing for the receiver, whose NowPlaying cache otherwise never learns about the row; (2) a pending optimistic song row (negative temp id) in the cache, meaning the frame is almost certainly the sender's own echo arriving BEFORE the POST resolved the temp id (BS broadcasts at commit), schedules nothing — the sender's own mutation refreshes NowPlaying on fulfillment. A skip-lane miss (a genuinely foreign row suppressed inside the sender's POST window) is repaired by that row's terminal-enrichment `update`: when an update's row is in NowPlaying but missing from a loaded entries cache, the update handler schedules the `Flowsheet` refetch the insert lane deferred. For new rows the backstop matters because: it keeps NowPlaying in step with a brand-new row, carries non-broadcast marker rows (breakpoints/talksets), and re-syncs offset-based pagination after page 0 grows. Bursts past `INSERT_PATCH_BURST_LIMIT` patches since the last backstop refetch skip the per-row patch and lean on that refetch, which a max-wait ceiling (`REFETCH_DEBOUNCE_MAX_WAIT_MS`) guarantees fires even while a sustained stream keeps re-arming the trailing debounce; a connection release drops (never flushes) any pending invalidate, because invalidating zero-subscriber queries makes RTK hard-delete the warm cache rather than mark it stale. An insert against an uninitialized cache emits `sse_cache_uninitialized` (once per connection, and never while the first entries GET is merely in flight — the public `/live` page never mounts the entries query, so this lane recurs there on every station-wide add by construction) and schedules the invalidate rather than silently no-oping; the invalidate refetches existing or previously-rejected substates but cannot start a never-initiated query.
...
**dj-site (PostHog at the time of this plan; errors additionally report to Sentry since docs/adr/0008)**:
- `safeCaptureException(err, { context: 'sse_parse_failure' | 'sse_dispatch_failure' | 'sse_connection_error' })` for the three error classes in the middleware — now via `lib/error-reporting.ts`, which delivers to both sinks, so the `$exception` events this plan assumed still arrive.
- `posthog.capture('sse_connected' | 'sse_disconnected' | 'sse_reconnecting', { surface, ... })` for connection-state visibility.
- `posthog.capture('sse_unknown_event_id', { surface, event_type, payload_id, current_show_id })` as a counter for tuning the unknown-id debounce policy. `surface` distinguishes dashboard vs live consumer, `event_type` is always `'update'` (the sole emitter is the update handler's unknown-id miss — refetch events carry no id and insert misses surface as `sse_cache_uninitialized` instead), `payload_id` lets ops identify whether specific rows are systematically out-of-cache, and `current_show_id` (read from the loaded cache, or `null` if none) helps spot whether unknown ids cluster around show boundaries. The insert path emits `sse_cache_uninitialized` (properties: `surface`, `event_type`, `payload_id` — no `current_show_id`, there is no loaded cache to read one from), at most once per connection, when an insert arrives before `getInfiniteEntries` has ever loaded (public `/live`, auth-gated dashboard skip) — the lane where RTK's `updateQueryData` would otherwise no-op silently.
...
- **dj-site**: ~~evaluate adopting Sentry alongside PostHog~~ — done, and "alongside" is literal: both sinks receive every error (docs/adr/0008).
