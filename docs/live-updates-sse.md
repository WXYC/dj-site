...
- **Out** (deferred to v1+): cross-tab leader election (each tab opens its own EventSource for now), reconnect-driven explicit refetch (5-minute safety poll covers gaps instead), `delete` SSE events from Backend-Service, `whoIsLive` SSE (no Backend-Service emission for show/DJ state yet), Sentry adoption in dj-site (PostHog stays).
