...
- **Out** (deferred to v1+): cross-tab leader election (each tab opens its own EventSource for now), reconnect-driven explicit refetch (5-minute safety poll covers gaps instead), `delete` SSE events from Backend-Service, `whoIsLive` SSE (no Backend-Service emission for show/DJ state yet), Sentry adoption in dj-site (since resolved: `lib/sentry.ts` owns error reporting — see docs/adr/0008; the `sse_*` analytics events below stay in PostHog).
...
**dj-site (PostHog at the time of this plan; the error classes moved to Sentry when docs/adr/0008 split error reporting from analytics)**:
- `safeCaptureException(err, { context: 'sse_parse_failure' | 'sse_dispatch_failure' | 'sse_connection_error' })` for the three error classes in the middleware (now delivered via `lib/sentry.ts`).
...
- **dj-site**: ~~evaluate adopting Sentry alongside PostHog~~ — done: `lib/sentry.ts` owns error reporting station-wide (docs/adr/0008).
