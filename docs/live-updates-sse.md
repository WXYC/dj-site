...
- **Shipped after v0** (dj-site#1118): `insert` SSE events. BS broadcasts new `entry_type='track'` rows (BS#1888) as `LiveFsInsertEvent` — the raw `FlowsheetEntryResponse` allow-list projection, NOT the converted read shape. The listener converts via `convertV2Entry`, splices the row into `getInfiniteEntries` page 0, and schedules the debounced `Flowsheet`+`NowPlaying` invalidate as the consistency backstop — for genuinely NEW ids only. Two lanes skip the splice and downgrade the backstop to a `NowPlaying`-only nudge (never `Flowsheet` — a refetch replays only the recorded pageParams, so a per-self-add refetch collapses the sender's optimistic rows past the page size out of the list during rapid adds): (1) an already-cached id, which is either the sender's own broadcast echo (the mutation pipeline owns that row; merging the raw pre-enrichment echo would downgrade enriched fields) or a receiver whose refetch raced the frame and delivered the row first — the nudge is load-bearing for the receiver, whose NowPlaying cache otherwise never learns about the row (later updates take the entries branch); (2) a pending optimistic song row (negative temp id) in the cache, meaning the frame is almost certainly the sender's own echo arriving BEFORE the POST resolved the temp id (BS broadcasts at commit), so splicing would render a duplicate beside the optimistic row. For new rows the backstop matters because: it keeps NowPlaying in step with a brand-new row, carries non-broadcast marker rows (breakpoints/talksets) the way the pre-insert unknown-id repair path used to, and re-syncs offset-based pagination after page 0 grows. Bursts past `INSERT_PATCH_BURST_LIMIT` per debounce window skip the per-row patch and lean on that refetch, which a max-wait ceiling (`REFETCH_DEBOUNCE_MAX_WAIT_MS`) guarantees fires even while a sustained stream keeps re-arming the trailing debounce; a connection release flushes (not drops) any pending invalidate. An insert against an uninitialized cache emits `sse_cache_uninitialized` (once per connection — the public `/live` page never mounts the entries query, so this lane recurs there on every station-wide add by construction) and schedules the invalidate rather than silently no-oping; the invalidate refetches existing or previously-rejected substates but cannot start a never-initiated query.
