...
|---|---|---|---|
| `/dashboard/md` | Music Department menu: the librarian's landing page | `rotation/musicmenu.jsp`, `libraryAdmin/libraryAdminLinks.jsp` | MD |
...
Naming follows `/wxycdb`'s own directory split (`libraryAdmin/`, `rotation/`) rather than nesting under `/dashboard/catalog`, which would collide confusingly with the unrelated modern `/dashboard/admin/catalog` (format + genre admin). `/dashboard/md` is the one route named for neither directory, because the menu it serves spans both: `musicmenu.jsp` lives in `rotation/` and `libraryAdminLinks.jsp` in `libraryAdmin/`. `/dashboard/music*` would read as a music-playing surface, and `/dashboard/admin/*` is the collision this paragraph already rules out.
