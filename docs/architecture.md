...
**The classic archived-show table is `Time · [indicators] · Artist · Song · Release · Label`.** `flowsheetRadioShowDisplayPublic.jsp` is five columns with no Time column at all, its rows being positional; Time is kept because an archived set is scanned by when things played, and the indicator gutter keeps the JSP's placement ahead of Artist. Marker rows keep their own Time cell and span the remaining five, so the Time column runs uninterrupted down the page and a talkset or a sign-on still says when it happened. The JSP spans its markers across all five of its own columns only because no row on that screen carries a time at all; that is a consequence of its column set, not an argument against one that exists here. The gutter's capsules come from `capsulesForSongEntry` — the resolver the classic flowsheet row already uses — in the public display's **ROTATION → REQUEST → EXCLUSIVE** order; the REQUEST-first order belongs to `flowsheetRadioShowModify.jsp`, a different screen. `Capsulable` speaks the live feed's `rotation`, so the V2 wire's `rotation_bin` is mapped at that call site rather than renamed on either side. `on_streaming` is three-state on the wire and only an explicit `false` earns EXCLUSIVE: a null means no linked library row and is not a claim about streaming availability.
