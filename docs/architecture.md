...
- **Server seed (public Server Component, unauthenticated GET).** `fetchBackendJson` in `lib/features/server-fetch.ts` throws on any failure — missing backend URL, network error/timeout, non-2xx, or an empty/non-JSON body. `fetchBackendSeed` wraps it and swallows the throw to `undefined`, so the page falls back to its client-fetched state instead of failing the request. A `"use cache"` accessor built on the same core must do the opposite and swallow the throw itself, because an error raised inside a cached scope does not surface at the call site — it escapes the caller's `try`/`catch` and aborts the render that awaited it, which under Cache Components means failing the production build, where the route is prerendered with no backend reachable. Such an accessor therefore resolves to `undefined` on every failure path. Caching that failure is safe only because `undefined` means "no seed" and the client query remains the authority; an accessor that failed to an empty array would instead serve a settled, wrongly-empty value for the whole `cacheLife` window, so a well-formed but unusable payload must resolve to `undefined` too, never to `[]`.
