...
**The classic forms adopt the rules, not the field group.** `src/components/experiences/classic/catalog/NewArtistForm.tsx` and `CreateLibraryCodeForm.tsx` duplicate this same field group and 409 policy, and already share `validateNewArtistNames` from `lib/features/catalog/chooserValidation.ts`. They cannot share the component: classic renders no MUI Joy, and `NewArtistFields` renders six Joy elements — which is also why `src/components/shared/inputs/` means "shared across modern surfaces" rather than across the two experiences (`ArtistSearchTypeahead`, already there, has three consumers and all of them are modern). What classic *can* adopt is everything in `lib/features/catalog/adminCreateArtistValidation.ts`, which is free of React and of Joy. PR 1 points both forms at `CODE_LETTERS_MAX_LENGTH` and `normalizeCodeLetters`; the ceilings they still do not enforce are a behavior change and belong with the divergence ticket, not here.
