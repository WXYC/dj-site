...
### 2026-08-19→22 — modal-dismissal e2e saga + CI event wedge
- Shard-3 e2e kept failing the two modal-close specs. Diagnosis ladder, each step CI-verified:
  1. Pathname guard (`ff43a7c6`) — insufficient: the intercepted `@information` slot receives NO re-render after `router.back()`; `usePathname` never delivers the new URL into that tree.
  2. Local closed-state + popstate listener (`a13f6323`) — still failed, which proved the forced CLICK never reaches the handler: trace shows the click performed at the button's coordinates, URL unchanged, no state change. Anything intercepting *inside* the Modal (incl. backdrop) would still fire onClose, so something outside/above swallows pointer events at that point. **Open question for the manual walkthrough: is the modal's close button click-dead for real users? One devtools inspection will name the overlay.**
  3. Escape-based dismissal in the page object (`3e50bee9`) — dismissal contract PASSES in a real browser (the return-to-catalog spec went green; local-state unmount + back() both work).
  4. Remaining failure was page-object debt: `expectModalHidden` still asserted rightbar-era "Now Playing inside the panel" (`d66b25cd` fixes it to assert the dialog hidden).
- **CI event wedge**: after `a13f6323`, pushes to `feat/catalog-md-visual` stopped creating ANY workflow runs (empty commit, close/reopen — nothing; sibling branches fire fine; PR head updates correctly). Workaround: `gh workflow run e2e-tests.yml --ref feat/catalog-md-visual`. Consequence: the PR's required pull_request-event checks can't attach, so #1243 may need an admin merge, a branch re-create, or GitHub support — same CI-signal-gap family as #1088.
