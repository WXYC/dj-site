...
2. **Set `NEXT_PUBLIC_STATION_SIGNUP_ADMIN_ENABLED=true` and redeploy dj-site.** In production this is a GitHub repository **variable** on WXYC/dj-site (`gh variable set NEXT_PUBLIC_STATION_SIGNUP_ADMIN_ENABLED --repo WXYC/dj-site --body true`) — the `deploy-production` job in `ci.yml` passes it into the build from `vars.*`, so after setting the variable, trigger a deploy by pushing to main — `deploy-production` runs only on a push to `main` (no manual dispatch), so merge the next PR or push an empty commit (`git commit --allow-empty -m "Redeploy for station-signup flag flip" && git push`). The manager-facing "Signup Passcode" tab and passcode panel now appear in the admin roster page. (Build-time flag — a rebuild/redeploy is required; setting the variable alone changes nothing until the next deploy.)
