...
Vitest config is in `vitest.config.mts`; the suite runs as node and jsdom projects (see [Environments](#environments)). Setup is split across two files so the `node` project never pays for DOM testing libraries it doesn't use. `tests/setup/vitest.setup.ts` runs for all three projects and handles:
- MSW server lifecycle (`beforeAll`/`afterEach`/`afterAll`)
- `localStorage` and `EventSource` mocks on `globalThis`

`tests/setup/vitest.setup.dom.ts` runs only for the `jsdom` and `jsdom-lib` projects (wired via each project's `setupFiles` in `vitest.config.mts`) and handles:
- The `@testing-library/jest-dom` matcher import and React Testing Library's `configure({ asyncUtilTimeout: 5000 })`
- `matchMedia`, `ResizeObserver`, `scrollIntoView`, and `IntersectionObserver` mocks (required by MUI and infinite-scroll components)
...
`eslint.config.mjs` enforces that: a `no-restricted-imports` override on `tests/unit/lib/**` and `tests/contract/**` bans `@/tests/helpers` in those tiers -- both the aliased specifier and a relative one resolving to the same module (`../helpers` from `tests/contract`, `../../helpers` from `tests/unit/lib`, one more `../` per extra level of nesting) -- since the barrel's render/store graph is paid for by importing any name from it, not just a particular one, and there's no per-name carve-out. The override's `ignores` does carve out the seven `DOM_DEPENDENT_LIB_TESTS` specs (`tests/setup/vitest-projects.ts`) by file -- they sit under `tests/unit/lib/**` but run in the `jsdom-lib` project, not `node`, so the "RTL costs nothing in this tier" justification for the ban doesn't hold for them. The error message names the deep path for every re-export that has one: fixtures from `@/tests/fixtures/fixtures`; constants and time utilities from `@/tests/helpers/constants` and `@/tests/helpers/time.vitest`; `describeConversion` from `@/tests/helpers/conversion-harness`; `server` and `handlers` from `@/tests/fakes/server` and `@/tests/fakes/handlers`; `createTestStore` from `@/tests/helpers/store`; `describeSlice` from `@/tests/helpers/slice-harness`; `describeApi` from `@/tests/helpers/api-harness`; `libraryTracksHandler`/`ONE_TRACK` from `@/tests/fakes/libraryTracks`; and `fakeRotationEndpoints`/`fakeRotationEndpointsWithGatedKill` from `@/tests/fakes/rotation`. Three re-exports have no entry in the message -- `render.tsx`, `field-value.ts`, and `component-harness.ts` -- because none has an RTL-free deep path to name: importing any of them pulls in `@testing-library/react`.

Component specs (`tests/integration/`) that need only `renderWithProviders` may deep-import it from `@/tests/helpers/render` -- paying the barrel's render/store cost there buys nothing extra, since the tier is already `jsdom` and RTL-dependent. Importing both the barrel and `@/tests/helpers/render` in the same spec is never useful, since the deep import buys nothing once the barrel is already paid for; in `tests/unit/lib` and `tests/contract` that combination is now a lint error for every spec but the seven `DOM_DEPENDENT_LIB_TESTS` pins the override's `ignores` carves out, while in `tests/integration` it remains a convention with no lint rule behind it.
...
See [Setup](#setup) for what each of `tests/setup/vitest.setup.ts` and `tests/setup/vitest.setup.dom.ts` covers and which projects wire it in. Only the latter appears in the `jsdom` and `jsdom-lib` projects' own `setupFiles`, so the `node` project never evaluates `@testing-library/jest-dom`. `@testing-library/react` is no longer a different story, but by two guarantees together, not one: the barrel has three RTL entrypoints -- `render.tsx`, `field-value.ts`, and `component-harness.ts` -- and the `no-restricted-imports` override on `tests/unit/lib/**` and `tests/contract/**` (see the Test Utilities section above) bans `@/tests/helpers` for every file in those tiers except the seven `DOM_DEPENDENT_LIB_TESTS` pins its `ignores` carves out (they run in `jsdom-lib`, not `node`), closing the only re-export path to them from the rest of those tiers; but that rule doesn't forbid a spec from deep-importing one of the three directly, or importing `@testing-library/react` itself, so `tests/unit/vitest.config.test.ts` also asserts that no file in the node project's file set does either. The `jsdom` project's `exclude` reuses `DOM_FREE_TIERS` rather than restating the two node-tier patterns. CI's `--changed` and `--shard` invocations operate across all projects' merged file sets.
...
- Putting a value in a field is *arrange* or *subject* — pick the tool accordingly. When a spec only needs the field to end up holding a value, call `setFieldValue(field, value)` from `tests/helpers`: one `change` event that neither focuses the field nor fires a keydown/keyup, and that writes straight through `disabled`, `readOnly`, and `maxLength`. If the spec asserts on anything the keystroke stream itself drives — a listbox opening, an append onto text already in the field, focus-driven behavior — use `user.type` instead and say so in a one-line comment next to the call. One subject `user.type` cannot serve: the timing *between* edits, as in a debounce window measured on a fake clock — it paces keystrokes on its own schedule and cannot hold the gap to a written-down duration, so those specs keep raw `fireEvent.change` interleaved with explicit `vi.advanceTimersByTime` calls, carrying the same one-line comment. Unlike the barrel/deep-import rule, this convention is enforced by prose and review rather than lint: the barrel rule polices a bare import statement with no legitimate exception, so a plain AST match suffices, while this rule's exception turns on whether the assertion genuinely depends on the keystroke stream — a judgment call a syntax-only rule can flag `fireEvent.change`'s presence for but can't verify. Revisit if raw `fireEvent.change` sites start reappearing the way the barrel convention drifted before it earned a lint rule.
