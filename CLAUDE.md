...
- **Always render through `renderWithProviders`** for component tests (never bare RTL `render`); use the slice / API / component / conversion harnesses. A `node`-tier spec (`tests/unit/lib`, `tests/contract`) never imports the `@/tests/helpers` barrel — fixtures/constants/conversion-harness/`server`/`createTestStore`/the slice and API harnesses all come from their own modules instead, lint-enforced with no per-name carve-out. See `docs/testing.md`.
