    ...
    if (session.data) {
      const normalizedSession = {
        ...
        user: {
          ...
          role: (session.data.user as typeof session.data.user & { role?: string }).role ?? undefined,
          banned: (session.data.user as typeof session.data.user & { banned?: boolean }).banned ?? undefined,
          banReason: (session.data.user as typeof session.data.user & { banReason?: string | null }).banReason ?? undefined,
          banExpires: (session.data.user as typeof session.data.user & { banExpires?: Date | null }).banExpires ?? undefined,
        },
      } as BetterAuthSession;

      // Convert session to authentication data (role is read directly from user record)
