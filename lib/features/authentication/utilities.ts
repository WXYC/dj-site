...
export type BetterAuthSession = {
  user: {
    ...
    createdAt?: Date;
    updatedAt?: Date;
    role?: string;  // User's WXYC role (e.g., "member", "dj", "musicDirector", "stationManager", "admin")
    banned?: boolean;
 ...
 * Uses session.user.role directly for authorization (Direct Role Model)
 */
...
/**
 * Convert better-auth session to AuthenticationData format (async version)
 * Uses session.user.role directly for authorization (Direct Role Model)
 * 
 * @deprecated Use betterAuthSessionToAuthenticationData instead - no longer needs async
 */
export async function betterAuthSessionToAuthenticationDataAsync(
  session: BetterAuthSession | null | undefined
): Promise<AuthenticationData> {
  // Delegate to synchronous version - no longer need async org lookups
  return betterAuthSessionToAuthenticationData(session);
}
