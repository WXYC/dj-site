...
export default async function getArtworkFromLastFM({ title, artist }) {
  let url =
    "https://ws.audioscrobbler.com/2.0/?type=release&per_page=1&page=1&" +
    "api_key=" +
    process.env.NEXT_PUBLIC_LAST_FM_KEY +
...
export async function getSongInfoFromLastFM({ title, artist }) {
  let url =
    "https://ws.audioscrobbler.com/2.0/?type=release&per_page=1&page=1&" +
    "api_key=" +
    process.env.NEXT_PUBLIC_LAST_FM_KEY +
