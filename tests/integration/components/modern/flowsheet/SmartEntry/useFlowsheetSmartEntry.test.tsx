...
import { useFlowsheetSmartEntry } from "@/src/components/experiences/modern/flowsheet/SmartEntry/useFlowsheetSmartEntry";
...
function Host() {
  const value = useFlowsheetSmartEntry();
  // Capture the latest hook API for test-body assertions without reassigning an
  // outer variable during render (react-compiler rule); the effect runs
  // post-commit, inside the act() the test already wraps state changes in.
  useLayoutEffect(() => {
    api = value;
  });
  return (
    <form onSubmit={(e) => value.submit(e)}>
      <textarea
        data-testid="composer"
        value={value.raw}
        onChange={(e) => value.onRawChange(e.target.value)}
      />
