...
vi.mock("@/src/components/experiences/classic/catalog/ReleaseCard", () => ({
  default: () => <div data-testid="release-card" />,
}));

import ClassicReleasePage from "@/app/dashboard/@classic/library/release/[id]/page";

const page = () => ClassicReleasePage({ params: Promise.resolve({ id: "53375" }) });

describe("Classic /dashboard/library/release/[id] page — libraryReleaseModify.jsp", () => {
  setUpClassicPageAuthorityEnv();

  it.each([
    { role: "musicDirector" as const, label: "a music director" },
    { role: "stationManager" as const, label: "a station manager" },
  ])("reaches the release editor for $label", async ({ role }) => {
    setUpClassicPageAuthority(role);

    await assertReachesClassicPage(page, "classic-main", "release-card");
  });

  it("redirects a DJ away — catalog search is DJ-facing, but editing what it found is not", async () => {
