...
// The page's own responsibility under test is the auth gate, not the
// table's RTK Query-backed content.
vi.mock("@/src/components/experiences/modern/Header/PageHeader", () => ({
  default: ({ title }: { title: string }) => <div data-testid="page-header">{title}</div>,
}));
vi.mock("@/src/components/experiences/modern/admin/shows/OpenShowsTable", () => ({
  default: () => <div data-testid="open-shows-table" />,
}));

import OpenShowsAdminPage from "@/app/dashboard/@modern/admin/shows/page";
...
describe("open shows admin page", () => {
  ...
  it("reaches the page for a music director", async () => {
    ...
    const result = await OpenShowsAdminPage();
    renderWithProviders(result);

    expect(mockRedirect).not.toHaveBeenCalled();
  });

  it("reaches the page for a station manager", async () => {
    mockGetSession.mockResolvedValue({ data: sessionData(null), error: null });
