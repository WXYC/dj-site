...
describe("rotation filing bench page", () => {
  ...
  it("reaches the page for a music director", async () => {
    ...
    const result = await RotationBenchPage();
  ...
  it("redirects a DJ away", async () => {
    ...
    await expect(RotationBenchPage()).rejects.toThrow("NEXT_REDIRECT:/dashboard");
  ...
  it("404s before any session read when the rotation admin flag is off", async () => {
    ...
    await expect(RotationBenchPage()).rejects.toThrow("NEXT_NOT_FOUND");
