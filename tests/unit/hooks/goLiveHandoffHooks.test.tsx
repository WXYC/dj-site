...
/**
 * The same roster read `useOpenShowHandoff` short-circuits on, exposed so the
 * prompt can decide whether a co-host join is worth offering. Read against the
 * real cache here, because the whole question is what that cache says.
 */
describe("useCallerIsOnAir (direct, real flowsheetApi cache)", () => {
  beforeEach(() => {
    mockUseRegistry.mockReturnValue({ loading: false, info: CURRENT_USER });
  });

  it("is true when the asking DJ is on the open show's roster alongside somebody else", async () => {
    const store = createTestStore();
    await primeCaches(store, {
      onAir: [
        createTestOnAirDJResponse({
    ...
    const { result } = renderHook(() => useCallerIsOnAir(), {
      wrapper: wrapperFor(store),
    });
  ...
  it("is false when somebody else is on air alone", async () => {
    const store = createTestStore();
    await primeCaches(store, {
      onAir: [createTestOnAirDJResponse({ id: "dj-sue-1", dj_name: "dj sue" })],
      entries: [
  ...
  it("is false when nobody is on air", async () => {
    const store = createTestStore();
    await primeCaches(store, { onAir: [], entries: [] });
  ...
  // Not the same question as `useOpenShowHandoff` returning null: that one also
  // answers null with no resolved user, and a false here has to mean "not on
  // air" rather than "cannot tell", or the prompt would offer a co-host join it
  // has no basis for.
  it("is false while the registry has not resolved a user yet", async () => {
    mockUseRegistry.mockReturnValue({ loading: true, info: null });
    const store = createTestStore();
    await primeCaches(store, {
      onAir: [
...
describe("useGoLiveHandoff — what the prompt says about the asking DJ", () => {
  beforeEach(() => {
    mockUseRegistry.mockReturnValue({ loading: false, info: CURRENT_USER });
    mockToastError.mockClear();
  });
  ...
  /**
   * The arrangement the whole co-host trap lives in, assembled from the real
   * caches: this DJ IS on the open show's roster, so the pre-emptive read
   * declines to prompt and the press goes to the server — which refuses it,
   * because the DJ is on the show without owning it. The prompt that opens must
   * carry that membership, or it will offer a join that does nothing.
   */
  it("marks a prompt opened by the server's refusal as coming from a DJ already on air", async () => {
    const store = createTestStore();
    await primeCaches(store, {
      onAir: [
        createTestOnAirDJResponse({
    ...
    // The pre-emptive read declined (this DJ is on the roster), so the server
    // is the only thing that could have opened this prompt.
    expect(goLive).toHaveBeenCalledTimes(1);
    expect(result.current.prompt?.callerIsOnAir).toBe(true);
  });

  it("marks the ordinary cache-known collision as coming from a DJ who is not on air", async () => {
    const store = createTestStore();
    await primeCaches(store, {
      onAir: [createTestOnAirDJResponse({ id: "dj-sue-1", dj_name: "dj sue" })],
      entries: [
    ...
    expect(goLive).not.toHaveBeenCalled();
    expect(result.current.prompt?.callerIsOnAir).toBe(false);
  });
});
